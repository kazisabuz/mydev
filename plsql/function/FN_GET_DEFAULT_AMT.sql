CREATE OR REPLACE FUNCTION FN_GET_DEFAULT_AMT(V_ENTITY_NUM     IN NUMBER,
...
BEGIN
  ...
  IF (W_PRODUCT_FOR_RUN_ACS = '1') OR (W_REPAY_FREQ = 'X') THEN
    IF (W_LMTLINE_LIMIT_EXPIRY_DATE < P_ASON_DATE) THEN
      V_OD_MONTH := TRUNC(MONTHS_BETWEEN(P_ASON_DATE,
                                         W_LMTLINE_LIMIT_EXPIRY_DATE + 1));
      IF V_OD_MONTH < 0 THEN
        V_OD_MONTH := 0;
      END IF;
    END IF;
  ...
  ELSE
    ...
    IF W_REPAY_FREQ = 'M' THEN
      W_REPAY_FREQ_IN_MON := 1;
    ELSIF W_REPAY_FREQ = 'Q' THEN
      W_REPAY_FREQ_IN_MON := 3;
    ELSIF W_REPAY_FREQ = 'H' THEN
      W_REPAY_FREQ_IN_MON := 6;
    ELSIF W_REPAY_FREQ = 'Y' THEN
      W_REPAY_FREQ_IN_MON := 12;
    ELSE
      W_REPAY_FREQ_IN_MON := 1;
    END IF;
  
    IF W_REPAY_START_DATE <= P_ASON_DATE THEN
      V_TOT_MONTH_FROM_REPAY_DATE := (FLOOR((TRUNC((MONTHS_BETWEEN(P_ASON_DATE,
                                                                   W_REPAY_START_DATE)))) /
                                            W_REPAY_FREQ_IN_MON) *
                                     W_REPAY_FREQ_IN_MON) +
                                     W_REPAY_FREQ_IN_MON;
    ...
    IF W_REPAY_AMOUNT = NULL OR W_REPAY_AMOUNT = 0 THEN
      W_REPAY_AMOUNT := 1;
    END IF;
  
    V_TOT_PAID_MONTH := FLOOR((W_REPAY_FREQ_IN_MON * V_PAID_AMOUNT) /
                              W_REPAY_AMOUNT);
  ...
  IF (V_PURPOSE_CODE = 'R' AND W_ASSET_CODE <> 'BL') THEN
    W_ASSET_CODE := 'UC';
  END IF;

  IF W_ASSET_CODE = 'UC' OR W_ASSET_CODE = 'SM' THEN
    V_DEFAULT_AMOUNT := 0;
  ELSIF (W_ASSET_CODE = 'SS' AND W_PRODUCT_FOR_RUN_ACS ='1' AND V_OD_MONTH < 7) THEN
    V_DEFAULT_AMOUNT := 0;
   ELSIF (W_ASSET_CODE = 'SS' AND W_PRODUCT_FOR_RUN_ACS ='0' AND V_OD_MONTH < 13) THEN
    V_DEFAULT_AMOUNT := 0;  
  ELSE
    V_DEFAULT_AMOUNT := ABS(W_OS_BALANCE);
  END IF;
