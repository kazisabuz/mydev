...
IS
    ...
    IS
        ...
        BEGIN
              SELECT D.RDODPENALDEFDTL_RUN_PERIOD,
               ...
               WHERE     D.RDODPENALDEFDTL_ENTITY_NUM =
            ...
            THEN
                FOR INDX
                    IN V_TABRDODPENDEFDTLS.FIRST .. V_TABRDODPENDEFDTLS.LAST
                LOOP
        ...
        IS
            ...
            V_NOOF_OD_INST            PKG_COMMON_TYPES.AMOUNT_T
                                          := P_NO_OF_OD_INST;
            V_TOT_NOOF_OD_INST        PKG_COMMON_TYPES.AMOUNT_T
                                          := P_TOT_OD_INST;
        BEGIN
            ...
            LOOP
                V_PENALTY_AMT :=
                      V_PENALTY_AMT
                    + (V_TOT_NOOF_OD_INST * V_PENAL_AMT_PER_OD_INST);
    ...
    BEGIN
        ...
        BEGIN
            SELECT COUNT (PBDCONT_INST_REC_FROM_AC)
              INTO W_DSA_COUNT
              FROM PBDCONTRACT
             WHERE     PBDCONT_INST_REC_FROM_AC =
        ...
        THEN
            ...
            BEGIN
                ...
                THEN
                    ...
                    PKG_AUTOPOST.PV_TRAN_REC (IDX).TRAN_CURR_CODE :=
                        P_ACNT_DETAILS.DEP_CURR;
                    PKG_AUTOPOST.PV_TRAN_REC (IDX).TRAN_VALUE_DATE :=
                        W_REC_DATE;
                    PKG_AUTOPOST.PV_TRAN_REC (IDX).TRAN_NARR_DTL1 :=
                ...
                IF     V_AUTO_RECV_CHG_AMT > 0
                   ...
                   AND TRIM (P_ACNT_DETAILS.AUTO_REC_CHGCD_INCOME_GL)
                           IS NOT NULL
                   AND TRIM (P_ACNT_DETAILS.AUTO_REC_CHGCD_VAT_GL)
                           IS NOT NULL
                THEN
                    IDX := IDX + 1;
            ...
            BEGIN
                ...
                W_RDINS_REC (IDX_RDINS).RDINS_TWDS_PENAL_CHGS :=
                    V_TOT_PENAL_AMT;
                ...
                W_RDINS_REC (IDX_RDINS).RDINS_REM3 :=
                    'DUE DATE : ' || W_REC_DATE;
                ...
                W_RDINS_REC (IDX_RDINS).RDINS_TWDS_CHG :=
                    NVL (V_RECV_CHG_AMT, 0);
                W_RDINS_REC (IDX_RDINS).RDINS_TWDS_VAT :=
                    NVL (V_RECV_VAT_AMT, 0);
            EXCEPTION
                WHEN OTHERS
                THEN
          ...
          SELECT V_ENTITY_NUM    ENTITY_NUM,
            ...
            FROM (SELECT O.*,
                    ...
                    FROM (SELECT M.*,
                                 ...
                                 B.ACNTBAL_BC_BAL
                                     ACNT_BAL
                            FROM (SELECT P.PBDCONT_BRN_CODE
                                             BRN_CODE,
                                         P.PBDCONT_PROD_CODE
                                             PROD_CODE,
                                         P.PBDCONT_DEP_AC_NUM
                                             INT_ACNT_NUM,
                                         TRIM (A.ACNTS_AC_TYPE)
                                             AC_TYPE,
                                         NVL (A.ACNTS_AC_SUB_TYPE, 0)
                                             AC_SUB_TYPE,
                                         P.PBDCONT_CONT_NUM
                                             CONT_NUM,
                                         TRIM (A.ACNTS_AC_NAME1)
                                             AC_NAME,
                                         ...
                                         CASE
                                             WHEN TRIM (
                                                      P.PBDCONT_INST_PAY_OPTION)
                                                      IS NULL
                                             THEN
                                                 NVL (
                                                     TRIM (
                                                         D.DEPPR_RD_DAY_OPTION),
                                                     '1')
                                             ELSE
                                                 TRIM (
                                                     P.PBDCONT_INST_PAY_OPTION)
                                         END
                                             INST_PAY_OPTION,
                                         NVL (TRIM (D.DEPPR_INST_CUTOFF_DAY),
                                              0)
                                             PROD_CUTOFF_DAY,
                                         NVL (
                                             TRIM (
                                         ...
                                         NVL (
                                             TRIM (
                                                 D.DEPPR_PENALTY_BSD_ON_PEN_DEF),
                                             '0')
                                   ...
                                   WHERE     NVL (TRIM (D.DEPPR_TYPE_OF_DEP),
                                         ...
                                         AND NVL (
                                                 ...
                                                 '0') =
                                             '1'
                                         AND NVL (P.PBDCONT_INST_REC_FROM_AC,
                                                  0) <>
                                             0) M,
                                 (  SELECT /*+ PARALLEL(R,8) */
                                           R.RDINS_RD_AC_NUM
                                           ...
                                           ROUND (
                                                 SUM (
                                                     NVL (R.RDINS_TWDS_INSTLMNT,
                                                          0))
                                               + SUM (
                                                     NVL (
                                                         R.RDINS_TWDS_PENAL_CHGS,
                                                         0))
    ...
    EXCEPTION
        ...
        THEN
            PKG_ERR_MSG :=
                   'error in INSERTING RDINS TABLE   '
                || SUBSTR (SQLERRM, 1, 500);
            RETURN FALSE;
    END;
