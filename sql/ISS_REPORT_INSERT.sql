/* Formatted on 5/28/2019 3:01:32 PM (QP5 v5.227.12220.39754) */
DECLARE
   V_ASON_DATE   DATE;
   V_CURR_DATE   DATE;
BEGIN
   V_ASON_DATE := TO_DATE ('DD-MON-RRRR', '30-APR-2019');
   V_CURR_DATE := TO_DATE ('DD-MON-RRRR', '28-MAY-2019');
               ...
               (SELECT NVL (SUM (FN_GET_ASON_ACBAL (1,
                ...
                (SELECT COUNT (ACNTS_INTERNAL_ACNUM)
                  ...
                  WHERE     ACNTS_ENTITY_NUM = 1
                        ...
                        AND ACNTS_OPENING_DATE <= V_ASON_DATE
                        --  AND P.PRODUCT_FOR_RUN_ACS = 1
                        AND (   ACNTS_CLOSURE_DATE IS NULL
                             OR ACNTS_CLOSURE_DATE > V_ASON_DATE)
                        AND ACNTS_BRN_CODE = IDX.MBRN_CODE)
                   COL55,
                (SELECT SUM ( (ABS (FN_GET_ASON_ACBAL (1,
                                                       ACNTS_INTERNAL_ACNUM,
                                                       ACNTS_CURR_CODE,
                                                       V_ASON_DATE,
                                                       V_CURR_DATE))))
                ...
                (SELECT SUM (ABS (FN_GET_ASON_ACBAL (1,
                  ...
                  WHERE     ACNTS_ENTITY_NUM = 1
                        ...
                        AND ACNTS_BRN_CODE = IDX.MBRN_CODE)
                   COL60,
                (SELECT SUM (ABS (FN_GET_ASON_ACBAL (ACNTS_ENTITY_NUM,
                                                     ACNTS_INTERNAL_ACNUM,
                                                     ACNTS_CURR_CODE,
                                                     V_ASON_DATE,
                                                     V_CURR_DATE)))
                ...
                (SELECT SUM (ABS (FN_GET_ASON_ACBAL (ACNTS_ENTITY_NUM,
                  ...
                  WHERE     ACNTS_ENTITY_NUM = 1
                        ...
                        AND ACNTS_BRN_CODE = IDX.MBRN_CODE)
                   COL65,
                (SELECT SUM (ABS (GET_SECURED_VALUE (ACNTS_INTERNAL_ACNUM,
                                                     V_ASON_DATE,
                                                     V_CURR_DATE,
                                                     ACNTS_CURR_CODE)))
                   FROM PRODUCTS, ACNTS
                ...
                0 COL127,
                (SELECT NVL (
                           SUM (ABS (FN_GET_ASON_ACBAL (ACNTS_ENTITY_NUM,
                                                        ACNTS_INTERNAL_ACNUM,
                                                        ACNTS_CURR_CODE,
                                                        V_ASON_DATE,
                                                        V_CURR_DATE))),
                ...
                (SELECT NVL (
                           ...
                           0)
                   FROM ACNTS
                  WHERE     ACNTS_OPENING_DATE <= V_ASON_DATE
                        --  AND P.PRODUCT_FOR_RUN_ACS = 1
                        AND (   ACNTS_CLOSURE_DATE IS NULL
                             OR ACNTS_CLOSURE_DATE > V_ASON_DATE)
                        AND ACNTS_ENTITY_NUM = 1
